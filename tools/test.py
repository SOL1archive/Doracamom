...
from projects.mmdet3d_plugin.bevformer.apis.od_occ_mtl_test import custom_multi_gpu_test
...
def parse_args():
    ...
    parser.add_argument('--use_old_custom_multi_gpu_test', action='store_true', help='use old test sh')
    parser.add_argument('--bad_condition_occ', action='store_true', help='bad condition_eval')
...
def main():
    ...
    if hasattr(cfg, 'plugin'):
        if cfg.plugin:
            ...
            if hasattr(cfg, 'plugin_dir'):
                ...
                for m in _module_dir[1:]:
                    _module_path = _module_path + '.' + m
                print(_module_path) #--'projects.mmdet3d_plugin'
                plg_lib = importlib.import_module(_module_path)

    # set cudnn_benchmark
    if cfg.get('cudnn_benchmark', False):
        torch.backends.cudnn.benchmark = True
    # set tf32 这里point_sampling的时候也禁用了，禁用PyTorch中的TensorFloat-32（TF32）加速功能
    ...
    data_loader = build_dataloader(
        ...
        dist=distributed,
        shuffle=False, #----false
        nonshuffler_sampler=cfg.data.nonshuffler_sampler, #---nonshuffler_sampler
    )
    ...
    if rank == 0:
        ...
        if args.eval:
            eval_kwargs = cfg.get('evaluation', {}).copy() #--dict(interval=1, pipeline=test_pipeline)
            ...
            eval_kwargs.update(dict(metric=args.eval, **kwargs)) #---{'pipeline': [{...}, {...}, {...}], 'metric': ['bbox'], 'jsonfile_prefix': 'test/bevformer_small/Wed_Jan_10_15_42_19_2024'}

            print(dataset.evaluate(outputs, **eval_kwargs))
            # _ = dataset.evaluate(outputs, **eval_kwargs)
